...
async function handler(m, { conn, args, text, usedPrefix, command }) {
    ...
    if ((user.money * 1) < count) return m.reply('Tu dinero no es suficiente para apostar!')
...
*Tu ${status}* ${status == 'Ganas!' ? `\n+${count * 2}` : status == 'Pierdes!' ? `\n-${count * 1}` : `\n+${Math.floor(count / 1.5)}`} de dinero
`.trim())
}
