...
let _sword = global.DATABASE._data.users[m.sender].sword
...
let _armor = global.DATABASE._data.users[m.sender].armor
...
case 'espada':
      if (global.DATABASE._data.users[m.sender].sword == 5) return conn.reply(m.chat, 'Tu *espada* esta al nivel máximo', m)
      if (global.DATABASE._data.users[m.sender].money > sword * 1) {
       	global.DATABASE._data.users[m.sender].sword += 1
           global.DATABASE._data.users[m.sender].sworddurability += sworddurability * 1
           global.DATABASE._data.users[m.sender].money -= sword * 1
...
case 'armadura':
      ...
      if (global.DATABASE._data.users[m.sender].money > armor * 1) {
           ...
           global.DATABASE._data.users[m.sender].armordurability += armordurability * 1
...
case 'caña':
      if (global.DATABASE._data.users[m.sender].rod == 1) return conn.reply(m.chat, 'Tu *caña* esta al nivel máximo', m)
