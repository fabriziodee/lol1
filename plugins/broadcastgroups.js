let handler  = async (m, { conn, text }) => {
  ...
  let member = (await conn.groupMetadata(groups)).participants.map(v => v.jid)
  ...
  for (let id of groups) await conn.copyNForward(id, await conn.loadMessage(m.chat, m.quoted.id), true, { contextInfo: { mentionedJid: member }})
