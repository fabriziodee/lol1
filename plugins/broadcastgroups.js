let handler  = async (m, { conn, text }) => {
  ...
  for (let id of groups) await conn.copyNForward(id, await conn.loadMessage(m.chat, m.quoted.id), true, { contextInfo: { mentionedJid: (await conn.groupMetadata(groups)).participants.map(v => v.jid) }})
