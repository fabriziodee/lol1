let handler  = async (m, { conn, text, participants }) => {
  ...
  for (let id of groups) await conn.copyNForward(id, await conn.loadMessage(m.chat, m.quoted.id), false, { quoted: m, contextInfo: { mentionedJid: participants.map(u => u.jid) }})
